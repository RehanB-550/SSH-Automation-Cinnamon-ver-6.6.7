...
### 2. Network Speed Permanently Showing 0.00 MB/s
...
### 3. Battery Drain From Aggressive Polling
The original 1-second refresh interval combined with full terminal clears (`\033[2J`) every cycle caused continuous I/O reads, CPU wakeups, and forced terminal repaints. On battery this was measurably destructive triggering fast battery drainage . The refresh interval was increased to `sleep(3)`, which reduces kernel file reads and terminal redraws by 66% with no meaningful loss in dashboard utility.  
##### This tool is for SERVER SETUP ONLY. Other connected remote devices can easily trigger "Sys-telemetry" without any battery problems as its solely running on SSH...
